...
from monitor_tutor.forms import MonitoresForm, TutoresForm
from interpretes.models import Interprete
from interpretes.forms import InterpretesForm
...
def adicionarAluno(request):
    ...
    if request.POST:
        ...
        form2 = EnderecosForm(request.POST)
        if form2.is_valid():
            form2.save()
            if form.is_valid():
                form.save()
            ...
            return HttpResponseRedirect('adicionarAluno')
    else:
        form = AlunosForm()
        form2 = EnderecosForm()
...
def adicionarMonitor(request, endereco_id):
    submitted = False
